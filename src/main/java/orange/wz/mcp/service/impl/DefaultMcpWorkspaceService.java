...
public final class DefaultMcpWorkspaceService implements McpWorkspaceService {
    ...
    private String updateNode(WzObject obj, Map<String, Object> operation) {
        ...
        return switch (op) {
            ...
            case "set_codepoints" -> {
                applySetCodepoints(obj, operation.get("codepoints"));
                yield op;
            }
            case "set_vector" -> {
    ...
    private void applySetCodepoints(WzObject obj, Object rawCodepoints) {
        if (!(obj instanceof WzStringProperty prop) || !(rawCodepoints instanceof List<?> values)) {
            throw new McpException("set_codepoints 只支持字符串节点和 codepoints 数组");
        }
        StringBuilder text = new StringBuilder();
        for (Object raw : values) {
            if (!(raw instanceof Number number)) {
                throw new McpException("codepoints 必须全部是数字");
            }
            int codepoint = number.intValue();
            if (!Character.isValidCodePoint(codepoint)) {
                throw new McpException("无效 Unicode codepoint: " + codepoint);
            }
            text.appendCodePoint(codepoint);
        }
        prop.setValue(text.toString());
        markChanged(prop);
    }
    ...
    private String normalizeUpdateOp(String op, Map<String, Object> operation) {
        if (op != null && !op.isBlank()) {
            return switch (op.trim().toLowerCase(Locale.ROOT)) {
                ...
                case "set_codepoints", "codepoints" -> "set_codepoints";
